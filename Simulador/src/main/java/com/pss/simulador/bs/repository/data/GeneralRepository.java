...
public interface GeneralRepository extends CrudRepository<General, Integer> {
	...
	@Query(value = "SELECT distinct g.nbDominio FROM General g WHERE g.stEstado = 1 AND g.fgEditable = 1")
	public abstract List<String> findAllDomainsActive();

	@Query(value = "SELECT distinct g.nbDominio FROM General g WHERE g.fgEditable = 1")
